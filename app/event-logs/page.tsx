...
import { eventLogsAPI, projectsAPI } from '@/services/api';
...
export default function EventLogsPage() {
  ...
  const { data: logs, isLoading } = useQuery({
    queryKey: ['event-logs', selectedProject],
    queryFn: async () => {
      const response = await eventLogsAPI.list(selectedProject);
      return response.data;
    },
  });
  ...
  const handleDownload = async () => {
    try {
      const response = await eventLogsAPI.download(selectedProject);
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-400 mt-2">View and manage your application event logs</p>
          </div>
          <button
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-lg overflow-hidden">
          {isLoading ? (
                  ...
                  {logs.map((log: EventLog) => (
                    <tr key={log.id} className="hover:bg-gray-800 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 py-1 text-xs font-medium rounded-full ${LEVEL_COLORS[log.level]}`}>
                          {log.level}
                        </span>
                      </td>
            ...
            <div className="text-center py-12">
              <p className="text-gray-400">No event logs found</p>
            </div>
          )}
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
          <div className="bg-gray-900 border border-gray-800 rounded-lg p-6 w-full max-w-2xl max-h-[80vh] overflow-y-auto">
            <div className="flex justify-between items-start mb-4">
              <h2 className="text-xl font-bold text-white">Event Log Details</h2>
              <button
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <span className={`inline-block mt-1 px-2 py-1 text-xs font-medium rounded-full ${LEVEL_COLORS[selectedLog.level]}`}>
                  {selectedLog.level}
                </span>
              </div>
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-white whitespace-pre-wrap bg-black border border-gray-800 p-3 rounded-md">{selectedLog.message}</p>
              </div>
